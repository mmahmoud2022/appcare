...
from app.models.doctor import Appointment, AppointmentStatusEnum, DoctorProfile, DoctorReview
...
class DetailedStatisticsResponse(BaseModel):
    ...
    pending_doctors: int
    active_users: int
    patients: List[PatientStatistics]
    doctors: List[DoctorStatistics]
...
@router.get("/statistics", response_model=DetailedStatisticsResponse)
async def get_statistics(
    ...
    **Requires admin privileges**
    ...
    Returns:
    - User counts (total, by role, active, pending)
    - Patient statistics with appointment counts by status
    - Doctor statistics with appointment counts by status and ratings
    """
    logger.info(f"Admin {current_admin.email} requesting detailed statistics")
    ...
    # Patient statistics with appointment counts
    patients_stats = []
    patients = db.query(User).filter(User.role == UserRole.PATIENT).all()
    
    for patient in patients:
        # Count all appointments by status
        ...
        # Total includes ALL appointments (pending, confirmed, completed, cancelled, no_show)
        total = db.query(Appointment).filter(
            Appointment.patient_id == patient.id
        ).count()
        
        patients_stats.append(PatientStatistics(
    ...
    # Doctor statistics with appointment counts and ratings
    doctors_stats = []
    doctor_profiles = db.query(DoctorProfile).all()
    ...
    for profile in doctor_profiles:
        ...
        # Calculate average rating from DoctorReview table
        avg_rating_result = db.query(func.avg(DoctorReview.rating)).filter(
            DoctorReview.doctor_id == profile.id
        ).scalar()
        
        avg_rating = float(avg_rating_result) if avg_rating_result else 0.0
        
        doctors_stats.append(DoctorStatistics(
            doctor_id=profile.id,
            doctor_name=f"Dr. {doctor_user.first_name} {doctor_user.last_name}",
            doctor_email=doctor_user.email,
            specialization=doctor_user.specialization or profile.specialty.value,
            pending_appointments=pending,
            confirmed_appointments=confirmed,
            completed_appointments=completed,
            cancelled_appointments=cancelled,
    ...
    logger.info(f"Statistics generated: {total_users} users, {len(patients_stats)} patients, {len(doctors_stats)} doctors")
