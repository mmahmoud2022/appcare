...
class PrescriptionStatusEnum(str, enum.Enum):
    """Statuts d'ordonnance électronique"""
    ISSUED = "issued"
    CANCELLED = "cancelled"
    EXPIRED = "expired"
...
class DoctorProfile(Base):
    ...
    schedule_entries = relationship("DoctorScheduleEntry", back_populates="doctor", cascade="all, delete-orphan")
    ...
    prescriptions = relationship("ElectronicPrescription", back_populates="doctor", cascade="all, delete-orphan")


class DoctorScheduleEntry(Base):
    """Configuration hebdomadaire des disponibilités du médecin"""
    ...
    break_duration = Column(Integer, nullable=False, default=0)
    consultation_type = Column(Enum(ConsultationTypeEnum, values_callable=lambda x: [e.value for e in x]), nullable=False)
    location = Column(String(255), nullable=True)

    created_at = Column(DateTime(timezone=True), server_default=func.now())

    doctor = relationship("DoctorProfile", back_populates="schedule_entries")


class DoctorBlockedSlot(Base):
    """Indisponibilités ponctuelles d'un médecin"""
...
class Appointment(Base):
    ...
    schedule_entry_id = Column(Integer, ForeignKey("doctor_schedule.id", ondelete="SET NULL"), nullable=True)
    ...
    prescriptions = relationship("ElectronicPrescription", back_populates="appointment")
    schedule_entry = relationship("DoctorScheduleEntry")
...
class ElectronicPrescription(Base):
    """Ordonnances électroniques émises par le médecin"""
    ...
    status = Column(Enum(PrescriptionStatusEnum, values_callable=lambda x: [e.value for e in x]), default=PrescriptionStatusEnum.ISSUED, nullable=False, index=True)
