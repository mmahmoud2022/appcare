<script lang="ts">
  ...
  const handleDeleteAppointment = async (appointment: Appointment) => {
    if (!confirm(`Êtes-vous sûr de vouloir supprimer le rendez-vous avec ${appointment.patient_first_name || appointment.patient?.first_name} ${appointment.patient_last_name || appointment.patient?.last_name} ?`)) {
      return;
    }
      ...
      <div class="p-6 space-y-4">
        <div>
          ...
          <p class="text-gray-700">
            {#if selectedAppointment.patient_first_name && selectedAppointment.patient_last_name}
              {selectedAppointment.patient_first_name} {selectedAppointment.patient_last_name}
            {:else if selectedAppointment.patient}
              {selectedAppointment.patient.first_name} {selectedAppointment.patient.last_name}
            {:else}
              N/A
            {/if}
          </p>
          <p class="text-sm text-gray-500">{selectedAppointment.patient?.email || selectedAppointment.patient_phone || ''}</p>
        </div>
        <div>
