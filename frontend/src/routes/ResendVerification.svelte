...
<script lang="ts">
  ...
  // Try to prefill email from query param
      ...
      {#if success}
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-5 mb-6 shadow-lg animate-scale-in">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-sm text-green-900 font-semibold">Un nouveau lien de vérification a été envoyé à <strong class="text-green-800">{email}</strong>.</p>
              <p class="text-sm text-green-800 font-medium mt-2">Vérifiez votre boîte de réception et suivez le lien pour activer votre compte.</p>
            </div>
          </div>
        </div>

        <button on:click={() => navigate('/login')} class="group relative w-full py-4 px-6 rounded-xl font-bold shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 overflow-hidden">
          <div class="absolute inset-0 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 opacity-90"></div>
          <div class="absolute inset-0 bg-white/20 backdrop-blur-sm"></div>
          <span class="relative flex items-center justify-center gap-2 text-white drop-shadow-lg text-lg">Retour à la connexion</span>
        </button>
      {:else}
        <form on:submit|preventDefault={handleSubmit} class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              <input type="email" id="email" bind:value={email} required class="w-full pl-4 pr-4 py-3.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 bg-white/50 backdrop-blur-sm font-medium" placeholder="votre.email@exemple.com" disabled={loading} />
            </div>
          </div>

          <button type="submit" disabled={loading} class="group relative w-full py-4 px-6 rounded-xl font-bold shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden">
            <div class="absolute inset-0 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 opacity-90"></div>
            <div class="absolute inset-0 bg-white/20 backdrop-blur-sm"></div>
            {#if loading}
            ...
            {:else}
              <span class="relative flex items-center justify-center gap-2 text-white drop-shadow-lg text-lg">Renvoyer le lien de vérification</span>
            {/if}
          </button>
        </form>

        <div class="mt-6 text-center">
          <button on:click={() => navigate('/login')} class="inline-flex items-center gap-2 text-gray-800 hover:text-emerald-700 font-semibold transition-all hover:gap-3">
